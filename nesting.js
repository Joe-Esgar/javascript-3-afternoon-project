...
function removeDuplicates() {
  ...
  for (let i = 0; i < workplaceAccidents.length; i++) {
    let dupFind = false;
    for (let k = 0; k < lessAccidents.length; k++) {
      if (workplaceAccidents[i] === lessAccidents[k]) {
        dupFind = true;
      }
    }
    if (dupFind === false) {
      lessAccidents.push(workplaceAccidents[i]);
    }
  }
  return (workplaceAccidents = lessAccidents);
}
////////// PROBLEM 3 //////////
